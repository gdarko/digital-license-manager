...
abstract class LicensePrivateStatus {
	/**
	...
	public static function statusToHtml( $status, $args = [] ) {
		...
		switch ( $status ) {
			...
			case self::SOLD:
				$markup = sprintf(
					'<div class="%s dlm-status-sold"><span class="dashicons dashicons-yes-alt"></span> %s</div>',
					$cssClass,
			...
			case self::DELIVERED:
				$markup = sprintf(
					'<div class="%s dlm-status-delivered"><span class="dashicons dashicons-yes-alt"></span> %s</div>',
					$cssClass,
