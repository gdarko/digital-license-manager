...
class DLM_WC_Helper_Product {
	 ...
	 */
	public static function create_variation_product( $product = null, $list_attributes, $list_variations ) {
		...
		foreach($list_attributes as $key => $options) {
			$attributes[] = self::create_product_attribute_object( $key, $options );
		}
		...
		$variations = array();
		foreach($list_variations as $variation_item) {
			$variation_object = self::create_product_variation_object($product->get_id(), $variation_item['sku'], $variation_item['price'], $variation_item['attributes']);
			if(!empty($variation_item['meta'])) {
				foreach($variation_item['meta'] as $key => $val) {
					$variation_object->add_meta_data($key, $val);
